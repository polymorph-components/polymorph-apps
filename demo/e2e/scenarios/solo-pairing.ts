...
import { act, assert, assertEquals, hookOn, sleep, SOLO_KEYS } from "../util.ts";

/** The solo page's driving root. */
const R = "__solo";
const solo = (page: Page, path: string, ...args: unknown[]) => hookOn(page, R, path, ...args);
...
const appFrame = (page: Page) => page.frameLocator("#solo-app iframe");
const todoRows = (page: Page) => appFrame(page).locator("ul.todo-list li");
