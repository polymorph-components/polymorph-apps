// Chrome's settings sheet: the one interaction with no component in it.
//
...
// itself). They are the USER'S words said in CHROME'S voice, so what
...
// outside chrome's fixed vocabulary onto the one position that is not
// spoofable.
 ...
 * `applyChromeHue` scopes it, deliberately: on `:root` the property
 * would inherit into every app region, so a component with a style
 * attribute could paint chrome's exact colour without reading it
 * (host/demo.ts `applyChromeHue`, and check-invariants.sh (c)). */
function anchorHue(page: Page): Promise<string> {
  return page.evaluate(() =>
    getComputedStyle(document.getElementById("chrome-strip")!)
      .getPropertyValue("--chrome-bg").trim()
  );
}
...
function stripIcon(page: Page): Promise<string> {
  return page.evaluate(() =>
    document.getElementById("chrome-settings")?.textContent ?? ""
  );
}

function idLines(page: Page): Promise<string[]> {
  return page.evaluate(() =>
    Array.from(document.querySelectorAll("#chrome-identity .id-lines .who")).map((e) =>
      e.textContent ?? ""
    )
  );
}
...
const scenario: Scenario = {
  ...
  async run(page, ctx) {
    await act("the settings button on the strip opens chrome's own sheet", async () => {
      await hook(page, "settings.openSheet");
      await waitForSheet(page, "settings", true);
      const text = await sheetText(page);
      assertIncludes(text, "Your chrome", "the sheet's heading");
      ...
      const foreign = await page.evaluate(() =>
        document.querySelectorAll("#chrome-drawer-inner .foreign").length
      );
      assertEquals(foreign, 0, "foreign-quoted text on the settings sheet");
    });

    await act("the strip names chrome's own sheet, and still names the APP upstairs", async () => {
      const strip = await page.evaluate(() => ({
        top: document.querySelector("#chrome-context .ctx-top")?.textContent ?? "",
        bottom: document.querySelector("#chrome-context .ctx-bottom")?.textContent ?? "",
      }));
      assertIncludes(strip.bottom, "chrome settings", "the bottom line");
      // Component identity is a property of what is INSTALLED, not of
      // which chrome sheet is open (host/demo.ts `topSurface`).
      assertIncludes(strip.top, "TodoMVC", "the top line while chrome's own sheet is open");
    });
    ...
    await act("picking a hue previews it on the ANCHOR immediately", async () => {
      ...
      assertIncludes(bottom, "chrome settings", "the bottom line during a live preview");
    });
    ...
    await act("a hand-edited glyph outside chrome's vocabulary falls back to ⛨", async () => {
      // The record is localStorage, so it IS hand-editable — that is the
      // threat model here, not a hypothetical. A free-text face could
      // spoof words in chrome's voice at the one position that cannot be
      // spoofed, so chrome renders only from CHROME_ICONS.
      ...
      const html = await injected.evaluate(() =>
        document.querySelector("#chrome-identity .id-lines .who")?.innerHTML ?? ""
      );
      assert(
        !html.includes("<img"),
    ...
    await act("the anchor colour is never ambient — the app region cannot resolve it", async () => {
      ...
      const ambient = await page.evaluate(() => {
        const read = (el: Element | null) =>
          el === null ? "" : getComputedStyle(el).getPropertyValue("--chrome-bg").trim();
        return {
          root: read(document.documentElement),
          body: read(document.body),
          // The regions chrome draws components into.
          panes: Array.from(document.querySelectorAll(".pane")).map(read),
        };
      });
      assertEquals(ambient.root, "", "--chrome-bg on the document root");
      assertEquals(ambient.body, "", "--chrome-bg on the body");
      for (const p of ambient.panes) {
        assertEquals(p, "", "--chrome-bg inherited into an app region");
      }
    });

    await act("the settings sheet is reachable ONLY from chrome's own pixels", async () => {
      ...
      const inStrip = await page.evaluate(() =>
        document.getElementById("chrome-settings")?.closest("#chrome-strip") !== null
      );
      assertEquals(inStrip, true, "the settings button lives inside the chrome strip");
      const inFrame = await page.evaluate(() =>
        document.querySelectorAll("iframe #chrome-settings").length
      );
